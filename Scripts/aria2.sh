...
Download_aria2() {
    ...
    echo -e "${Info} 请输入Aria2安装包路径(必须是.tar.gz格式):"
    read -e -p "路径: " ARIA2_PACKAGE

    if [[ ! -f "${ARIA2_PACKAGE}" ]]; then
        echo -e "${Error} 安装包文件不存在！"
        exit 1
    fi

    if [[ "${ARIA2_PACKAGE}" != *.tar.gz ]]; then
        echo -e "${Error} 安装包必须是.tar.gz格式！"
        exit 1
    fi

    tar -zxf "${ARIA2_PACKAGE}"
    [[ ! -s "aria2c" ]] && echo -e "${Error} Aria2 解压失败！" && exit 1
