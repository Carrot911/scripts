...
- Support for the all known SteamUser versions (9 to 23)
- Support for the games that are making the use of steamclient(64).dll

Installation (x86) [Windows (Classic)]:
+++++++++++++++++++++++++++++
...
- Configure cream_api.ini (see "Configuration notes" below)
- Enjoy

Installation (x64) [Windows (Classic)]:
+++++++++++++++++++++++++++++
- Download and extract the DLC you want
...
Installation (x86/x64) [Windows (Proxy)]:
+++++++++++++++++++++++++++++
- Download and extract the DLC you want
- Rename the steam_api.dll (steam_api64.dll) from the unlocker folder to the one that is used by the game *
  * winmm.dll is the most common one. winhttp.dll and version.dll variants are also available
- Copy the renamed library and cream_api.ini to the game folder (put it next to the executable file) *
- Configure cream_api.ini (see "Configuration notes" below)
- Enjoy

Installation (x86/x64) [Linux]:
+++++++++++++++++++++++++++
- Download and extract the DLC you want
- Locate the current game installation directory (<InstalledGameName> -> "Manage" -> "Browse local files")
- Find and rename the original libsteam_api.so to libsteam_api_o.so (see "Configuration notes" below)
- Copy libsteam_api.so and cream_api.ini to the game folder *
- Configure cream_api.ini (see "Configuration notes" below)
- Enjoy
...
- Download and extract the DLC you want
- Locate the current game installation directory (<InstalledGameName> -> "Manage" -> "Browse local files")
- Open the main game package (RMB -> "Show package contents")
- Find and rename the original libsteam_api.dylib to libsteam_api_o.dylib
- Copy libsteam_api.dylib and cream_api.ini to the game folder *
- Configure cream_api.ini (see "Configuration notes" below)
- Enjoy
...
Configuration notes:
++++++++++++++++++++
- (Windows only) Some games do have a special check of steam_api.dll/steam_api64.dll, so if it's modified, game won't start anymore (e.g. Magicka)(also some games are checking if there is an *.ini file inside the directory).
  ...
  As a side note, some games are checking for the specific files in the game directory. Fill in the "filestohide" option with the list of files you want to hide (separated with comma).
- (Linux only) In order to find out if the original libsteam_api.so from Valve is either x64 or x86, open the folder where the library is inside the terminal and
  paste the following command:

  od -An -t x1 -j 4 -N 1 libsteam_api.so

  If it prints 01, then the library is 32-bit (x86) and if it prints 02, then the library is 64-bit (x64)
...
v5.3.0.0:
- Fixed the crash caused by additional check inside the "SteamInternal_FindOrCreateGameServerInterface" function (fixes Hearts of Iron IV)
- Added support for the latest Steamworks SDK v1.60 (API version: 8.97.99.70)

v5.2.0.0:
- [Windows] Added support for the system library proxy mode (now it's possible to rename the CreamAPI to: winmm.dll, winhttp.dll or version.dll)
- [Linux] Fixed the crash caused by the certain linker option
- Added support for the latest Steamworks SDK v1.59 (API version: 8.63.11.84)
- Added some missing exports
- Added "filestohide" option
- Fixed the case when "GetDlcByIndex" function could cause a crash
- Fixed the DLC ID parsing issue
- Fixed a few tiny bugs

v5.1.0.0:
- The first cross-platform release!
- Added Linux CreamAPI build (x86, x64)
- Added MacOS CreamAPI build (ARM64/x86_64)
- Added support for DLC.txt (aka MrGoldberg's Steam emu format) as a backup option to unlock DLCs
- Fixed the typo in code while reading the SteamClient version (fixes "Space Engineers" and more)
- Fixed a few log message typos (log build only)
- Updated an INI reader library
- A few smaller code enhancements

v5.0.0.0:
- Almost completely refactored the base code (at least 70%)
- Added support for the latest Steamworks SDK including a few previous ones (v1.51 to v1.58a, API version: 8.33.9.23).
- Enhanced an "extraprotection" module (Automobilista 2 and maybe more games)
- Enhanced the logging (log build only)
- Fixed the crashes in some games
- Added more internal code checks for efficiency purposes (e.g. the check if DLC ID is a real number)
- Replaced an INI reader library with the modern one
- Updated an "appid" value check. Now if it's equal to "0", it will try reading the Steam environment variable.
- Updated the logic of the "unlockall" option. Now it should not conflict if the "DLC" section is not empty
